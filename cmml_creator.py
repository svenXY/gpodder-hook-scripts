...
PODCAST_LIST = [ (LINUX_OUTLAWS, True), (RADIOTUX, True) ]
...
class gPodderHooks(object):
    def on_episode_downloaded(self, episode):
        ...
        if (channel_title.startswith(LINUX_OUTLAWS) and
            [state for pc, state in PODCAST_LIST if pc == LINUX_OUTLAWS]):
        	create_cmml_linux_outlaws(html, audio_file)

        elif (channel_title.startswith(RADIOTUX) and
            [state for pc, state in PODCAST_LIST if pc == RADIOTUX]):
