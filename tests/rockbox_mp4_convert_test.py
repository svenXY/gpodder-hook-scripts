...
class TestRockboxMP4Convert(unittest.TestCase):
    def setUp(self):
        ...
        self.rb_hook = hook.gPodderHooks() 

    def tearDown(self):
        self.client._db.close()

        converted_file = self.rb_hook._get_rockbox_filename(self.filename)
    ...
    def test_file_renaming(self):
        ...
        self.assertEqual(self.rb_hook._get_rockbox_filename(self.filename),
            os.path.join(os.path.dirname(self.filename), 'MattCutts_2011U.mpg'))
        
    def test_calc_resolution(self):
        resolution = self.rb_hook._calc_resolution(512, 288, 224.0, 176.0)
        self.assertEqual(resolution, (224, 126))

        resolution = self.rb_hook._calc_resolution(1024, 768, 224.0, 176.0)
    ...
    def test_mp4convert(self):
        ...
        new_filename = self.rb_hook._convert_mp4(self.filename, hook.DEFAULT_PARAMS)
